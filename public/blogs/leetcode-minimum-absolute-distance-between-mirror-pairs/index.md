...
class Solution {
    public int minMirrorPairDistance(int[] nums) {
        ...
        for(int j=0;j<nums.length;j++){
            Integer a = map.getOrDefault(nums[j], -1);
            if(a>-1){
                ans=Math.min(ans,j-a);
            }
            map.put(reviveNum(nums[j]), j);
        }
        return ans==Integer.MAX_VALUE?-1:ans;
    }
    private int reviveNum(int targetNum){
        int revive=0;
        for(;targetNum>0;targetNum/=10){
            revive=revive*10+targetNum%10;
        }
        return revive ;
    }
}
